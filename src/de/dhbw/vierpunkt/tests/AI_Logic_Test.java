...
public class AI_Logic_Test {
	...
	public static class GameLogic {
		...
		public GameLogic() {
			// Array durchlaufen und mit Nullen fuellen + move auf false setzen,
			// da
		 ...
		 * 
		 * 
		 * eine funktion die das match, game saved! Wenn wir Daten vom bekommen
		 * (gegner)
		 */
		...
		/**
		 * Getter fuer field. Erwartet x und y - Wert und liefert den Wert im
		 * Array zurueck!
		 **/
		 ...
		 * 
		 * @return 1 --> Spieler 1 oder 2 --> Spieler 2 oder Agent 3 -->
		 *         UNENTSCHIEDEN 0 --> noch kein Gewinner
		 */

		/**
		 * Prueft, ob Chip eingeworfen werden kann gibt -1(keine valide
		 * Position) oder Zeile zurueck!
		 **/
		public int validPosition(int x) {
			int temp = 0;
			// Spalte muss im richtigen Bereich > 0 & kleiner max. Anzahl
			// SPALTEN
			if (x > -1 && x <= COLUMN) {
				for (int y = 0; y <= ROW; y++) {
			...
			// temp nur zurueckgeben, wenn noch keine Zeile returned wurde, war
			// eine
		...
		public static int evaluate(GameLogic game) { // bewertet die
														// Spielsituation
			...
			for (int x = 0; x <= game.getColumn(); x++) {
				for (int y = 0; y <= game.getRow(); y++) {
					// System.out.println("Jetzt sind wir in den
					// for-Schleifen");
			...
			// System.out.println("agentCount2 " + agentCount2 + " agentCount3 "
			// + agentCount3 + " oppCount2 " + oppCount2 + " oppCount3 " +
			// oppCount3);
		...
		public int isGameWinner() {
			int gameWinner = 0; // Game wird gewonnen, wenn 2 oder 3 Matches
								// gewonnen werden

			if (s1 >= 2) { // Gegner hat 2 oder 3 Matches gewonnen -> Spiel
							// gewonnen
				gameWinner = 1;
			} else if (s2 >= 2) { // Agent hat 2 oder 3 Matches gewonnen ->
									// Spiel gewonnen
				gameWinner = 0;
			}
		...
		private static int isRow(int[][] field, int player, int x, int y, int dx, int dy) {
			...
			// cnt++; // Count ist 1
			// if ( (0<=(y+1*dy)) && ((y+1*dy)<=ROW) && (0<=(x+1*dx)) &&
			// ((x+1*dx)<=COLUMN) ) {
			// if (field[y+1*dy][x+1*dx] == player) { // n�chstes auch
			// cnt++; // Count ist 2
			// if ( (0<=(y+2*dy)) && ((y+2*dy)<=ROW) && (0<=(x+2*dx)) &&
			// ((x+2*dx)<=COLUMN) ) {
			// if (field[y+2*dy][x+2*dx] == player) { // �bern�chstes auch
			// cnt++; // Count ist 3
			// if ( (0<=(y+3*dy)) && ((y+3*dy)<=ROW) && (0<=(x+3*dx)) &&
			// ((x+3*dx)<=COLUMN) ) {
			// if (field[y+3*dy][x+3*dx] == player) { // 3.n�chstes auch
			// cnt++; // Count ist 4
			// }
			// }
			// }
		...
		public static int getAlphaBeta(GameLogic game, int depth, int alpha, int beta) {
			...
			if (depth == 0)
				return evaluate(game); // evaluate game situation by counting
										// the coins in row/ in diagonal/ in
										// column
			else {
				// calculate all possible moves
				for (int i = 0; i <= game.getColumn(); i++) { // try out all
																// columns
					game_tmp = game.getDemoGame(); // copy current game
													// situation to simulate
													// possible moves without
													// actually changing the
													// play field
					if (game_tmp.validPosition(i) != -1) { // position is valid
						minimax_tmp = getAlphaBeta(game_tmp, depth - 1, alpha, beta);
						if (game.getCurrentPlayer() == 2) { // our agent is
															// playing
							minimax_curr = Math.max(minimax_tmp, minimax_curr); // is
																				// the
																				// new
																				// value
																				// of
							...
							minimax_curr = Math.min(minimax_tmp, minimax_curr); // is
																				// the
																				// new
																				// value
																				// of
		...
		public static int calcMove(GameLogic game, int depth) { // method to be
																// evoked by
																// game main to
																// find the best
																// possible move
			...
			for (int i = 0; i <= game.getColumn(); i++)
				if (game.validPosition(i) != -1) { // position is valid
					game_tmp = game.getDemoGame(); // copy current game
													// situation to simulate
													// moves without actually
													// changing the play field
					game_tmp.setChip(i); // simulate that our agent is placing
											// coins in column i
					values[i] = getAlphaBeta(game_tmp, depth, (int) Double.NEGATIVE_INFINITY,
			...
			for (int j = 0; j <= game.getColumn(); j++) {
				if ((values[j] >= max) && (game.validPosition(j) != -1)) { // position
																			// is
																			// valid
																			// and
																			// current
			...
			return move; // best possible move after all columns have been
							// evaluated
		...
		public static void main(String[] args) throws IOException {
			...
			/***
			 * Achtung: Nur zum Testen! While Schleife bricht nur ab, wenn
			 * Gegner gewinnt -> Also im Optimalfall nie, wenn wir anfangen zu
			 * spielen! => Endlosschleife!
			 */
			int test = 0;

			// BufferedReader br = new BufferedReader(new
			// InputStreamReader(System.in)); // f�r manuellen Spielmodus
